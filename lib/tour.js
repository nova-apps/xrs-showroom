 ...
 *   - PER-ARROW OVERRIDE: when the plan isn't to scale, the derived bearings
 *     are wrong and calibrating one arrow throws off the others. So each node
 *     also carries `hotspots: { [targetId]: lon }` — an EXPLICIT longitude for
 *     that one arrow, calibrated by aiming the crosshair at the neighbor. When
 *     present it wins; otherwise the arrow falls back to northOffset+bearing.
 *     This lets the operator calibrate every arrow independently.
 *   - Calibration: the operator centers the view on a chosen neighbor and we
 *     store that camera lon as the arrow's override (one arrow at a time).
 */
...
export function emptyNode(id) {
  return {
    ...
    hotspots: {},   // targetId → explicit arrow longitude (deg), per-arrow calib
  };
}

/** Coerce a stored hotspot map into `{ [targetId]: normalizedDeg }`. */
function normalizeHotspots(raw) {
  if (!raw || typeof raw !== 'object') return {};
  const out = {};
  for (const [tid, lon] of Object.entries(raw)) {
    const n = Number(lon);
    if (Number.isFinite(n)) out[tid] = normalizeDeg(n);
  }
  return out;
}
...
export function normalizeTour(tour) {
  ...
  for (const [id, n] of Object.entries(rawNodes)) {
    ...
    nodes[id] = {
      ...
      hotspots: normalizeHotspots(n.hotspots),
    };
  }
  // Second pass: a link is only valid if its target survived; prune hotspot
  // overrides to the surviving links too.
  for (const n of Object.values(nodes)) {
    n.links = [...new Set(n.links)].filter((t) => t !== n.id && nodes[t]);
    const hs = {};
    for (const tid of n.links) {
      if (Number.isFinite(n.hotspots?.[tid])) hs[tid] = n.hotspots[tid];
    }
    n.hotspots = hs;
  }
...
/**
 * Longitude (degrees) inside `node`'s image where the hotspot toward `target`
 * should be rendered. A per-arrow override (node.hotspots[target.id]) wins;
 * otherwise it falls back to the derived model: northOffset + plan bearing.
 */
export function hotspotLon(node, target) {
  const override = node?.hotspots?.[target?.id];
  if (Number.isFinite(override)) return normalizeDeg(override);
  return normalizeDeg((Number(node?.northOffset) || 0) + planBearingDeg(node, target));
}

/** True if this specific arrow was calibrated by hand (has an override). */
export function isHotspotCalibrated(node, targetId) {
  return Number.isFinite(node?.hotspots?.[targetId]);
}

/**
 * Calibration for one arrow: the operator centered the view on `targetId`
 * while standing at `node`, camera at `lon`. The arrow simply lives where they
 * looked, so the override IS that longitude.
 */
export function hotspotLonFromCam(lon) {
  return normalizeDeg(Number(lon) || 0);
}

/**
 * Camera longitude to open `to` with when arriving from `from`. We face
 * "forward" — away from where we came — which is the calibrated arrow back
 * toward `from` (hotspotLon(to, from)) turned 180°. Since hotspotLon already
 * prefers per-arrow calibration and falls back to northOffset+bearing, this
 * stays correct for both hand-calibrated and legacy tours.
 */
export function arrivalLon(from, to, fromLon) {
  if (from && to && from.id !== to.id) {
    return normalizeDeg(hotspotLon(to, from) + 180);
  }
  // Degenerate fallback: keep the heading we had.
  return normalizeDeg(Number(fromLon) || 0);
}
